...
The 14th field is the interesting one. **`on_favorites` does not take effect**: the vendor's example writes the YAML 1.1 bareword `on_favorites: yes`, and JSON cannot emit a bareword — a quoted `"yes"` is a string. That is the one documented field where the JSON-as-YAML approach cannot reproduce their example, and the command now warns rather than letting it look applied.
